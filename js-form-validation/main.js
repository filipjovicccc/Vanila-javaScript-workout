...
inputs.forEach((element) => {             //prolazim ih kroz elemente
  element.addEventListener("change", (e) => {
    ...
    if (inputValue.length > 4) {
      errors[inputName] = [];
      switch (inputName) {  //swict i caseovi
        case "ime_prezime":
        ...
        case "email":
          if (!validateEmail(inputValue)) {
            errors[inputName].push("Ne ispravna email adresa");
          }
          break;
        case "ponovi_lozinku":
...
const populateErrors = () => {
  ...
  for (let key of Object.keys(errors)) {
    // let key of object zapamtiti ovu metodu
